	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		elseif QBCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
            if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
                ...
                local imageurl = ("https://cfx-nui-dt-inventory/html/images/%s.png"):format(itemData.name)
                ...
                QBCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
                exports['dt-mdt']:CreateWeaponInfo(serial, imageurl, notes, owner, weapClass, weapModel)
...
RegisterNetEvent('dt-inventory:server:SaveStashItems', function(stashId, items)
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info, item.created) then
				...
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				TriggerClientEvent('dt-inventory:client:giveAnim', src)
				TriggerClientEvent('dt-inventory:client:giveAnim', target)
			else
...
QBCore.Functions.CreateCallback('dt-inventory:server:GetStashItems', function(source, cb, stashId)
...
RegisterNetEvent('inventory:server:addTrunkItems', function()
	print('inventory:server:addTrunkItems has been deprecated please use exports[\'dt-inventory\']:addTrunkItems(plate, items)')
end)
RegisterNetEvent('inventory:server:addGloveboxItems', function()
	print('inventory:server:addGloveboxItems has been deprecated please use exports[\'dt-inventory\']:addGloveboxItems(plate, items)')
end)